...
設定について:
    ...
    注意: レイアウトは会社ごとに違うので、新しい会社は画面で一度確認すること。
    設定JSONは再現性の材料そのものなので、消さずに残す。
...
方針:
    捨てるのは柱（全ページで反復する文言）だけ。（ページ除外もヘッダー・フッターの
    座標カットも行わない。分母＝総文数・総ページ数は全ページ・全文で数え、
    冊ごとの人の判断を前処理から排除する）
    それ以外は種別（本文／大／小／極小／表）を付けて全部残す。
    前処理で捨てたものは戻らないが、種別を付けておけば分析のときに選べる。

処理の中身:
    抽出ロジックは core.py（画面と共通）。
"""
import collections
import csv
import os
...
def load_keywords():
    ...
    画面とバッチが必ず同じ語で抽出する。実装は cachekit に1本化。
    """
...
def main():
    ...
    for pdf in pdfs:
        ...
        secs = time.time() - t0
        # 分母は全ページ・全文（ページ除外を適用しないので、除外ページ数の列も持たない）
        ...
        # 注意: 絵文字は Windows のコンソール（cp932）で落ちるので、表示用の文字列には使わない
    ...
    def write_csv(path, cols, data):
        with path.open("w", encoding="utf-8-sig", newline="") as f:
            # 注意: extrasaction="ignore"：core.extract_doc が付ける印（ページまたぎ など）は列にしない
    ...
    # 注意: openpyxl は使わない：ワークシート参照を絶対パスで書き、KH Coder のパーサが読めない
    #       xlsxwriter は Excel と同じ相対パスで書くので、こちらを使う
    ...
    try:
        ...
        # 注意: Excel や KH Coder で開いたままだと Windows がファイルをロックしていて書けない
        #       他の出力は全部済んでいるので、止めずに知らせる
