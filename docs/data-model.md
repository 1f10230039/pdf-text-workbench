...
| キー | 単位 | 意味 |
|---|---|---|
| `excluded` | 文 | ロゴ・参照表記など、文書の記述でないものの除外。全単位が除外されたブロックは結合の隣接判定で透過になる（段の継ぎ目に残ったページ番号が結合を妨げないように） |
| `joins` / `splits` | ブロック | 機械が切りすぎた箇所の結合／癒着した箇所の分割（互いに逆操作）。`splits` は表の行ブロックにも適用でき、セル群の癒着を境目で分けられる（[ADR-0010](adr/0010-table-cell-group-splits.md)） |
| `kinds` | ブロック | フォントサイズからの種別自動判定の上書き |
| `manual_order` | ページ | 読み順の手動指定 |
| `tables` / `table_off` | 範囲／ページ | 表の手動指定／表の自動検出の停止（誤検出のほか、本物の表でもセルの折返しが行の復元で混ざる場合に使う） |
| `unit_merges` / `unit_excludes` | 抽出単位 | 抽出単位への文の追加／単位の除外（L2 層。理由コードに 目次/表紙/章扉/対照表/ヘッダー/フッター/リンク/用語注記 を含む計13種）。追加は「既に別の単位に属する文」を対象にできない（二重カウント防止） |
