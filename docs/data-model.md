...
| キー | 既定 | 意味 |
|---|---|---|
| `header_y` / `footer_margin` | 35 / 45 | ページ上端・下端の切り取り境界（pt）。文書ごとに診断して調整する |
| `body_size` | 自動推定 | 本文のフォントサイズ。種別（本文／大／小／極小）判定の基準 |
| `repeat_ratio` | 0.3 | 全ページの一定割合以上で同位置・同文言の行を「柱」として除去 |
...
| キー | 単位 | 意味 |
|---|---|---|
| `skip_pages` | ページ | 表紙・目次などページごとの除外 |
...
| `unit_merges` / `unit_excludes` | 抽出単位 | 抽出単位への文の追加／単位の除外（L2 層） |
| `unit_checks` | 抽出単位 | 目視確認の記録（確認日時と所要秒数） |
| `boundary_check` | 文書 | 「切り取りと除外」の判断の記録。`ヘッダー`・`フッター`・`ページ` の3キー（＋日付）が揃うと点検完了 |
...
| `文書名.cands.json` | 本文サイズの推定値・除外ページの自動候補 |
| `文書名.meta.json` | 一覧表示用のメタ（単位数・確認数など） |
| `文書名.boundary.json` | 境界診断の結果（落ちた行の分類と座標。署名つき） |
