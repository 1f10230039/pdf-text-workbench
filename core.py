...
OP_REASONS = {
    ...
    "splits": [
        ...
        # 2026-09-02 箇条書きに限らず散文のセルにも一般化（Q社 2025 p133 の目標｜実績表）
    ...
    "table_off": [
        ...
        # 2026-09-02 追加（T社 2024 p13 のマテリアリティ一覧）。本物の表でも、複数行に折り返す
        # セルが多いと行の復元がセル同士の行を文の途中で混ぜる。検出をやめれば列ブロックとして
        # 各セルが完全な文言で取れる
    ...
    "unit_excludes": [
        ...
        # 2026-09-02 追加（T社の「※7 大規模言語モデル。」など）。⚠️ 範囲を定義する注記
...
def clean_text(text: str) -> str:
    ...
    t = text.replace("　", " ")
    t = re.sub(r"[ \t]+", " ", t)
    # ⚫（U+26AB）は 2026-09-02 追加（O社 2025 の箇条書きで実測。●と同じビュレット）
    t = re.sub(r"[■●▲◆•⚫]+", "", t)
    ...
    # span の中に改行を埋め込んでいるPDFがある（R社 2024 p20 ほか、2026-09-02 に14冊で実測。
    # 放置すると KH Coder 用 txt の行が文の途中で割れる）。行の結合と同じ扱いにする＝
...
def apply_table_splits(table_rows: list[dict], st: Settings,
    ...
    （_row_raw）で作り直す。`line` は**前半に残す行数**（行ブロック内の並び＝列→上から
    の1始まりで、line 行目までが前半・その次からが後半。2026-09-02 監査で実挙動に合わせて
    説明を修正）。分けた**両方の断片**に続けて照合するので、
