...
理由は、UI（ui/app.py）とバッチ（pdf2txt.py）が完全に同じ処理を通ることを保証するため。
UIで詰めた設定が、そのままバッチで再現されないと意味がない。

1ページから取り出すもの：
  ・ページ番号／ページラベル（印刷上の番号。get_label() が返す）
  ・セクション（直近の大見出しを引き継ぐ）
  ・抽出設定（Settings）。文書ごとに JSON で保存して再現に使う

前処理で破壊的に捨てるのは反復行（repeated_lines）だけ。ページ単位の除外も、
座標によるヘッダー／フッターの切り取りも行わない。ヒットに混ざった柱・番号は
L2 の unit_excludes（理由コード付き）で単位側から外す。
"""
...
# --- 既定値 -------------------------------------------------------------
# 実際の報告書PDFを目視して決めた値。
# 注意: 文書ごとにレイアウトが違うので、これは「出発点」であって正解ではない。
DEFAULTS = {
    # 注意: 座標でヘッダー・フッターを切る設定（`header_y`・`footer_margin`）は持たない。
    #       ページ番号と同じ高さに本文が回り込む文書があり、座標では分けられず
    #       文書ごとの判断が必須になるため。切らずに全部残し、ヒットに柱・番号が混ざったときは
    #       L2 の unit_excludes（理由：ヘッダー／フッター）で単位側から外す。
    #       61冊で計測したところ、切らずに残る 15,901 行のうち検索語を含むものは 0 件だった
    # 全ページのこの割合以上で「同じ位置に同じ文言」が現れる行は柱（ランニングヘッド・ナビ・
    # 社名入りフッター）とみなして捨てる（0で無効。→ repeated_lines）。
    # これが唯一の「捨てる」規則で、文書ごとの判断を挟まず全文書に一律で効く
    "repeat_ratio": 0.3,
    ...
    "join_gap": 1.2,        # 同じ行の続きとみなす横の隙間（フォントサイズの何倍まで）。
                            # 「原則1」＋「人権擁護の支持と尊重」のように、1文字ぶん空けて
                            # 横に並ぶラベルを1行にするための値。段の間は実測で 3.0〜4.0倍
                            # 空くので、1.2 なら段どうしが繋がることはない
    ...
    # --- 表（→ find_page_tables / table_row_groups） ---
    # 罫線で区切られた表を検出し、行ごとに1ブロックに組み直す。
    ...
    # 注意: 番号だけのリストにしないのは、後から見て p45 が章扉だったのか判断ミスだったのかが
    #       分からず、監査記録として使えないため。
    ...
    # 注意: pt が無いと、同じページに同じ文言が2つあるとき（見出しと参照リンクなど）
    ...
    "auto_join": True,
    # --- 抽出単位（L2。→「抽出単位について」/ extract_units） ---
    ...
    # 確認モードで「確認した」印。抽出結果は変えない（進み具合と、1サイトあたりの確認時間の実測）
    "unit_checks": [],      # [{"page": ページ, "hit": ヒット文, "秒": 確認にかかった秒, "日": "YYYY-MM-DD"}]
}

# 「表」は、罫線で区切られた表の1行を1ブロックにしたもの。
...
# 残らない。2文書目以降で同じ手順を踏んだと言えなければ、文書間の比較が成立しない。
# → 手順を先に決め、各項目について「どう結論したか」を必ず記録する形にした。
#   これで「全文書に同一の手順を適用し、各文書の判断を記録として残した」と言える。
#
# 注意: `must` は「必ず外す」ではなく「外すのが既定」という意味。
#       全項目を必須にすると、中身を見ずに埋める動作が身につく（確認ダイアログと同じ問題）。
#       だから判断が割れる項目は `must=False` にして、
...
TASKS = [
    ...
    {"key": "編集方針", "label": "編集方針・報告範囲", "must": False,
     "note": "「報告にあたって」「レポートについて」等も同じ。"
             "⚠️ 参照した制度名（TCFD/TNFD/GRI 等）が書かれていることが多く、"
...
# 理由の内訳が出れば、その文書のレイアウトの性質として説明できる。
#   例）結合30件のうち28件が「段またぎ」→ 2段組みの文書だからで、恣意的な操作ではない
#
# 注意: 理由は必須にしない。未設定でも動く。強制すると、埋めるために適当な理由を
#       選ぶようになって、記録の意味がむしろ落ちる。
# 注意: 選択肢はレイアウト由来の事実に寄せてある（「読みやすいから」のような主観を並べない）。
#       結合の分類が group_lines の3条件（same_size / same_col / next_line）に対応しているのは、
#       「機械がなぜ切ったか」を書くのが一番説明しやすいため。
OP_REASONS = {
    ...
    "splits": [
        ...
        # 表の行の分割（apply_table_splits）で使う理由。箇条書きに限らず散文のセルも対象
        {"key": "セル群の癒着", "label": "表の1行に、独立したセル群が複数連結されていた",
         "note": "列見出し（目標／実績など）を異にする自己完結したセル群（箇条書き・散文を問わない）が"
                 "1行に連結された表（ラベルが行内で再掲される場合を含む）。セル群の境目で分け、"
                 "ヒットを含むセル群を単位にする（単一セル内の箇条書きは行全体のまま）"},
    ],
    ...
    # 除外した単位：ここは「そもそも文書の記述でないもの」に限る（層の分け方は README）
    "excluded": [
    ...
    # 表の検出をやめたページ：罫線はあるが表ではないもの、または行の復元が使えないもの
    "table_off": [
        {"key": "図解の枠", "label": "表ではなく図解の枠線だった（組織図・フロー図など）"},
        {"key": "レイアウトの枠", "label": "ページ全体や段組みを囲む枠線だった"},
        # 本物の表でも、複数行に折り返すセルが多いと、行の復元がセル同士の行を文の途中で
        # 混ぜてしまう。検出をやめれば列ブロックとして各セルが完全な文言で取れる
        {"key": "復元が崩れる表", "label": "本物の表だが、セルの折返しが行の復元で混ざる",
         "note": "検出をやめて列ブロック（セル単位の完全な文言）で取る。行のまとまりが必要なら「足す（表の続き）」で束ねる"},
    ],
    ...
    "unit_excludes": [
        ...
        # 「※7 大規模言語モデル。」のような言い換えだけの注記が対象。範囲を定義する注記
        # （「※1 国産LLM、マルチ生成AIなど」＝何を指すかの列挙）は内容なので残す
        {"key": "用語注記", "label": "用語の言い換え・略語展開だけの注記",
         "note": "「※ LLM：大規模言語モデル」のような、本文の語をほどいただけで新しい情報が無いもの"},
        ...
        # 目次・表紙のような構成要素のページに出たヒットは、ページごとではなく
        # その単位だけを理由付きで外す
        {"key": "目次", "label": "目次の項目",
        ...
        # ヘッダー・フッター帯の行は座標で切らずに残すので、ヒットに混ざったらここで外す
        ...
        # リンク一覧の項目（「〈製品名〉: 製品・ソリューション」など）が対象
...
def reason_counts(st: Settings) -> dict:
    """操作ごとの理由の内訳。件数だけでは説明にならないので、この内訳を記録として残す。"""
    out = {}
...
# 段落を単位にしない理由
# ---------------------------------------------------------------------------
#   ・1ページ＝1単位にして手で区切りを入れる方式は、数十冊の規模では手作業が回らない
#   ・字下げ・短い行・太字などの規則で段落を切る方式は、文書ごとに破綻し、
#     ある文書で直すと別の文書で副作用が出た。「段落」の定義そのものが人の感覚で、
#     機械的に全文書へ当てはまらない
#   → 段落の規則も段落モードも持たず、文とページだけにしてある。
#
# なぜ文とページか
# ---------------------------------------------------------------------------
#   文   … 句点で切れる。全文書で同じ定義になり、KH Coder の標準の単位でもある。
...
#          崩れやすいのでページ単位で抽出する」としている。実測では本文は1ページあたり
#          中央値 340〜1,480 字＝日本語で2〜5段落ぶん。「節」くらいの粒度で、でかすぎはしない。
#   → 狭い端（文）と広い端（ページ）の両方で見て、両方で出る結果を採る。
#     中間の「段落」は作らない。
#
...
#                    注意: H1〜H5 の見出しの文字列は KH Coder 3.Alpha.7 以降、語の集計から外れる
#                    （旧掲示板 No.3231 の樋口氏の回答）ので、文書名や「p12」は語に混ざらない。
#   外部変数_文書.csv ／ 外部変数_ページ.csv … txt の H1／H2 と同じ順・同じ数の行（社・年・群・ページ）
#
# ブロック（④）は「文を壊さないための下処理」として残る（横割れの結合・ページまたぎ・柱・表の行）。
# ブロックの切れ目は文の切れ目を強制するだけで、集計単位ではない。
#
...
#   ④ ＋座標 ←いまここ
#
# ③で足りなかった実例（KPIバッジが3枚のカードに1つずつ並ぶページ）：
#
#     6.1pt  KPI   x0= 55.5    ← Planet positive のカード
#     6.1pt  KPI   x0=311.5    ← Prosperity positive のカード
#     6.1pt  KPI   x0=567.6    ← People positive のカード
#
# 1つ消すと3つとも消えた。全文書で「同じページ・同じ文言・同じpt」は115箇所
# （最大12個重複）あり、例外ではなく普通に起きる。
#
# 座標を鍵に入れても、壊れやすくはならない。
# 「パラメータを変えると単位の切れ方が変わって指し先を見失う」ことは起きるが、
# ブロックの切れ方が変われば文言も変わるので、座標があろうと無かろうと一致しない。
# → 座標を足しても新しい壊れ方は増えず、同じ文言の別の実体を区別できるだけ増える。
#
# 注意: ただし当たらなくなったルールは黙って無視される（＝直したはずが直っていない）。
#       これは元からある性質だが、条件が増えるぶん起きやすくなる。
#       → `analyze_page` が当たらなかったルールを返し、画面で警告を出すようにした。
#
# 注意: 座標を持たない古いルールは、これまでどおり位置を問わず一致する。
...
#
# 注意: これは「前処理での恣意的な取捨選択」そのもの。設定JSONに全件が残るので、
#       除外した文言のリストと件数はそのまま監査記録として出せる。
#       増えすぎたら、個々の除外ではなく方法のほうを疑うこと。
...
class Excluder:
    ...
    注意: pt / 位置 を持たない古いルールは、それぞれその条件を問わずに一致する。
    """
...
class KindOverride:
    ...
    def get(self, text: str, page: int, bbox: list | None = None) -> str | None:
        # 注意: ページ指定のあるルールを先に見る。全ページ指定より個別指定が優先
...
@dataclass
class Settings:
    """1文書ぶんの抽出設定。そのまま JSON にして保存する（＝再現の材料）。

    座標カットの設定（`header_y`・`footer_margin`）は持たない（→ DEFAULTS の注記）。
...
def _migrate_skip_pages(v) -> list[dict]:
    ...
    古い設定JSONは `[1, 2, 3]` の形。黙って捨てずに理由を空欄で引き継ぐ。
...
def task_status(st: Settings) -> list[dict]:
    """手順ごとの「どう結論したか」。画面・バッチ・書き出しが同じものを見るために共通化する。
...
def unfinished(st: Settings) -> dict:
    ...
    手順チェックリスト（TASKS）は現在の前処理では使わない。分母が全ページ・全文なので
    「外すべきページを外したか」という問い自体が無く、催促するとかえって使わない作業へ
    誘導してしまう。task_status / TASKS は過去の設定JSONを読むために残してある。
    """
    return {}
...
def detect_body_size(doc) -> float:
    ...
    文書ごとに本文のptが違う（9.0だったり10.5だったり）ので決め打ちにしない。
    注意: 図表の多い文書ではキャプションのほうが多くなる可能性がある。
...
# --- 柱（全ページで同じ位置に同じ文言）を捨てる ----------------------------
# 「全ページの3割以上で、同じ x（±3pt）に同じ文言」が現れる行を柱とみなして捨てる。
# 例：ページ右端の章名ナビ（1ページ36単位×190ページ）、タイトル入りフッター。
# 文書ごとに1回数えるだけの機械的な規則。章名の見出しは章の中でしか繰り返さない（3割未満）。
# 割合は `repeat_ratio`（既定 0.3。0 で無効）。少なくとも5ページに現れることも条件にする。
# これが唯一の「捨てる」規則で、文書ごとの判断を挟まない。反復しない番号・章名が
# ヒットに混ざったときは L2 の unit_excludes（ヘッダー／フッター）で外す
_repeat_cache: dict[tuple, frozenset] = {}


def _line_key(text: str, x0: float, y0: float) -> tuple:
    # 注意: y は鍵に入れない。右端の章名ナビは、その章の小項目の数で上下にずれるので、
    #       同じ y には来ない。同じ x（列）に同じ文言が3割以上のページで出れば柱とみなす
...
def _line_size(spans) -> float:
    ...
    日本語の文字を含む span のうち、最も文字数の多いもののサイズ。日本語が無ければ最多の span。
    注意: 単純に「最も文字数の多い span」で決めると、英数字だけ別のフォント・別のサイズで
          組むデザイン（日本語 7.8pt／英数字 8.4pt など）で英数字の多い行だけ 8.4pt になり、
          段落の途中で「大」に化けて段落が割れる。数字や英単語は本文の一部なので、
          行の大きさは日本語の文字で決める。
    """
    jp, other = [], []
    for sp in spans:
...
def drop_duplicate_blocks(groups: list[dict]) -> list[dict]:
    ...
    注意: 行単位で消してはいけない。上の例のように改行位置が1文字ずれるので、
          行単位だと一致する行だけが消えて、残ったブロックが虫食いになる（実際そうなった）。
          ブロックにまとめてから、空白を無視して比較する。
...
def merge_row_fragments(lines: list[dict], st: Settings) -> list[dict]:
    ...
    実例：`「Respect every voice.」` と `「Think big. Be ` が同じ y座標なのに別の行になる。
    これを繋がないまま次の工程へ渡すと、文の途中に句点が補われて文が割れる。

    注意: 横に繋ぐのは危険な操作でもある。2段組みでは左段と右段が同じ y にあるため、
          雑に繋ぐと pdfminer で起きた「左右が混ざる」問題を自分で再現することになる。
          だから、隣り合う断片の横の隙間で判定する。

    実測（ある報告書の全110ページ／隙間はフォントサイズ比）:

        -6.5 〜 -1.5倍   38件  すべて同じテキストの重複描画（縁取り効果）。繋いではいけない
...
def group_lines(lines: list[dict], st: Settings) -> list[list[dict]]:
    ...
    for ln in lines:
        if cur:
            prev = cur[-1]
            # 注意: 許容差は size_tol（既定0.6）と広めに取る。英数字だけの行（別フォントで
            #       0.6pt 大きい）が段落の途中で別ブロックになるのを防ぐため。見出しは太字・
            #       字下げ・空きで別に切れるので、広げても見出しが本文にくっつくことはまず無い
...
def _block_break(cur: list[dict], ln: dict) -> bool:
    ...
    ブロックの切れ目は文の切れ目になる（→ to_units が句点を補う）。だから、ここで見るのは
    「この行は前の行の文の続きではない」と機械的に言える2つだけ：
      ① 箇条書き・番号の頭で始まる行（新しい項目）
      ② 太字だけの行と、そうでない行の境目（本文と同じ pt で太さだけで見出しにしたデザイン）
    字下げ・前の行が短い・列の右端 などの「段落」の規則は持たない。レイアウトから段落を
    当てる規則は文書ごとに破綻し、直すたびに別の文書で副作用が出るため
...
def kind_of(size: float, body: float, st: Settings) -> str:
    ...
    注意: 「極小」を最初に見る。放っておくと「小」に紛れて見えなくなるため。
       ...
       注意: 閾値は文書ごとに違う。上の例では全体の0.5%だったが、
             サムネイルを多用する文書ではもっと多くなる。画面で件数を見て決めること。
    """
...
def to_units(text: str, st: Settings) -> list[str]:
    ...
    句点で終わらない単位（見出し・ラベル）にも句点を補い、単位の形を揃える。

    注意: 入力は「句点を補う前」の生テキスト。手でブロックを繋ぐとき（`joins`）も、
...
def clean_text(text: str) -> str:
    """`to_units` の整形部分（句点で切る前段）。

    どの置換も句点「。」を作らず・消さず・またがない。だから
    ...
    t = text.replace("　", " ")
    t = re.sub(r"[ \t]+", " ", t)
    # ⚫（U+26AB）は ● と同じビュレットとして使われる
    t = re.sub(r"[■●▲◆•⚫]+", "", t)
    # 私用領域（U+E000〜F8FF）の文字＝Wingdings 等の記号フォントで描いた箇条書きの点・矢印。
    # 文字としての意味は無く、残すと KH Coder で謎の1文字語になる（U+F09F など）
    t = re.sub(r"[\ue000-\uf8ff]+", "", t)
    # 制御文字（目次のリーダー線や箇条書きを制御コードで描いているPDFがある）と、
    # 同じ記号の3回以上の連続（「…………」「........」のリーダー）を落とす
    t = re.sub(r"[\x00-\x08\x0b-\x1f\x7f]", "", t)
    # span の中に改行を埋め込んでいるPDFがある（放置すると KH Coder 用 txt の行が
    # 文の途中で割れる）。行の結合と同じ扱いにする＝
    # 日本語は消すだけで繋がる。英数字の間だけは語がくっつかないよう空白にする
    t = re.sub(r"(?<=[0-9A-Za-z])\n(?=[0-9A-Za-z])", " ", t)
    t = t.replace("\n", "")
...
def reading_order(groups: list[dict], page_width: float) -> list[dict]:
    ...
    PyMuPDF が返すブロックの順序は描画順であって、読む順ではない。
    実データでは、写真の上に重ねた見出しが最後に描かれることがある：

        gid0 グローバル共通の価値観「Our Way」   (y=165)   ← 本文が先
        …
        gid6 CEOメッセージ                      (y= 50)   ← ページ最上部の見出しが最後

    注意: これは表示の問題では済まない。セクションは「直近の大見出しを引き継ぐ」方式
    なので、見出しが本文より後ろにあると、本文が前ページのセクションに
    割り当てられてしまう。

    並べ替え方：x0 の隙間で「列」に切り、列を左から順に、列の中は上から順に。
    単純な y ソートでは2段組みの左右が交互になるので使えない。
    """
    if len(groups) < 2:
        return groups
...
def apply_manual_order(groups: list[dict], st: Settings, pageno: int) -> list[dict]:
    ...
    先に順序を確定させないと隣が変わる）。だから `keys` に入るのは結合前の生text。

    注意: 保存した後でパラメータ（JOIN_GAP など）を触ると、ブロックの切れ方が変わって
          `keys` に無いブロックが現れる。そういうブロックは直前のブロックの後ろに置くので、
          並びが全部飛ぶことはない（元の位置の近くに留まる）。

    同じ文言のブロックが複数あるページでの割り当て。
    `{生text: 順位}` の辞書で持つと、同じ文言があるとき最後の1つで上書きされ、全部が
    同じ順位になる。実例：`KPI` が3つあるページで `keys` が `[…, KPI, B, KPI, …]`
    のとき、1つ目の `KPI` まで B の後ろへ飛んでしまう。

    → `keys` を先頭から見て、同じ文言のブロックへ出てくる順に割り当てる。
    座標を足さずに直せる。文言が同じなら、どちらがどちらに割り当たっても出力は同じ
    （区別できないものを区別する必要がない）ので、順番に配れば足りる。
    """
    keys = None
...
def apply_joins(groups: list[dict], st: Settings, pageno: int) -> list[dict]:
    ...
    注意: `a` も `b` も「結合前」の生text であること。既に結合されたブロックの
    連結後テキストを渡すと、どのブロックにも当たらず黙って無視される。
    ...
    """
    # 注意: `part_boxes` は各パーツの結合前の左上座標。種別・集計単位の区切りは
    ...
    last_live = None                # 結合先＝直前の透過でないブロック
    for g in groups:
        # 全単位が除外されたブロックは透過させる。段の継ぎ目にページ番号が挟まると
        # 結合の隣接が切れるため。「データから消す」（excluded）を選んだブロックは、
        # 結合の流れの上でも居ないものとして扱う
        if g.get("全除外"):
            g["parts"] = [g["raw"]]
            g["part_boxes"] = [g["bbox"][:2]]
...
def apply_splits(line_groups: list[list[dict]], st: Settings,
    ...
    背景：本文と同じptで、色や太さだけで見出しにしたデザインでは、
    見出しの行が下の本文と同じブロックに入ることがある。見出しは句点で終わらないので、
    句点で切り直すと見出し＋本文が1つの文になってしまう。同じ見た目の見出しでも、
    独立したブロックなら単独の「文」になる——つまり扱いがPDFの内部構造の偶然で
    ...
    注意: 結合（joins）より前、ブロックが group_lines から出てきた直後の行のまとまりに
    適用する。だから `text` は結合前の生text（joins の a/b と同じ流儀）。
    分けた両方の断片に続けて照合するので、1ブロックを3つ以上に分けることもできる
    （2本目のルールは、1本目で分けた断片の生textを名指しする）。
    分けた後は前半にも照合を続ける。後ろ半分だけに続けると、「後ろの境目 → 前の境目」
    の順に操作したとき、前半を名指しした2本目のルールがどこにも当たらず黙って効かない。
    UIは見えているブロックの境目を選ばせるので、エンジン側が操作の順序に依存してはいけない。
...
#
# 解きたい問題（指標名と数値が横に並ぶ、健康経営の指標表のようなもの）
# ---------------------------------------------------------------------------
# 表のセルは1つずつ別のブロックになるので、③④の規則（同じ列・行送り）で繋ぐと
# 列ごとに繋がる。「定期健康診断受診率」と「99.9%」は別の列にあるので、
# 1ページが142ブロックに割れ、指標名と数値が二度と出会わなかった。
# 人が読むときの単位は行（指標名＋年度ごとの値）なので、そちらに合わせる。
#
# どうやって表を見つけるか
# ---------------------------------------------------------------------------
# PyMuPDF の `page.find_tables()` を使う。既定の `lines_strict` は、PDFの中の罫線（線分）
# だけを手がかりにセルを組み立てる（pdfplumber 由来の規則的な処理。学習モデルではない）。
#   ・塗りつぶしの矩形は見ない（`lines` にすると見る。ある指標表では列が16本に割れた）
...
# 行への組み直し
# ---------------------------------------------------------------------------
# 検出された表の横罫線の y 座標で行バンドを、セルの左端の x 座標で列バンドを作り、
# 表の範囲内の各行（テキスト行）を、その中心座標が入るバンドに割り当てる。
#   ・同じ行バンドのものを、列の順（左→右）に、同じ列の中は上から並べて1ブロックにする
#   ・列と列の間は半角スペース、同じ列の中で折り返した行は詰めて繋ぐ（日本語）
# セル単位で割り当てないのは、1列ぶんのラベルがセル1つにまとまっているPDFがあるため
# （「Scope 1／Scope 2／Scope 3」が1セルに入っている例）。行バンドで割ると正しく別行に入る。
# 代償として、複数行に折り返した結合セル（「生活習慣病・／がん対策」）は2行に割れる。
# 注意: 表の中にある表（外枠）は捨てる。ページ全体を囲む枠が 2×2 の表に見えることがある。
#
# 出力は種別 `表`。ptでは決まらない種別なので、他の4つとは別の軸だが、
...
# 表検出の結果キャッシュ。
# find_tables が処理時間の9割超を占める（実測：extract_doc 6.5秒のうち約6秒）。
# 検出結果は「そのページ＋表関連の設定（strategy・table_off・手動の範囲）」だけで決まる
# 純関数なので、その組でキャッシュする。確認モードの L1 編集（文の除外・結合など）は
# 表の設定を変えない＝再解析で表検出を全ページぶんスキップでき、体感が数秒→1秒弱になる。
# 注意: 返す前に deepcopy する（呼び出し側やJSON化で書き換えられてもキャッシュを汚さない）
...
def find_page_tables(page, st: Settings, pageno: int) -> list[dict]:
    ...
    if pageno not in st.table_off_set():
        ...
        # 塗りだけで行や列を区切る表（シナリオ表など）を拾うため。
        # 注意: 罫線表のあるページでは使わない（塗りが邪魔をして列が細かく割れる）
    ...
    for i, t in enumerate(found):
        ...
        if not outer:
            keep.append(t)
    if len(_table_cache) > 30000:      # 数十冊×数百ページでも余る上限。溢れたら作り直し
        _table_cache.clear()
...
def table_row_groups(tables: list[dict], lines: list[dict],
    ...
    セルの中の行を、行送りの近さ（1.8×pt 以内）で「ひとつながり（run）」にまとめてから決める。
      ・run が1つだけ → 縦中央寄せの見出し（「重点施策」の列）や折り返した文言（「疾病による休業の状／況」）
        → 1つの文言として、セルの先頭の行バンドに入れる（結合セルの見出しは先頭行に属するとみなす）
      ・run が複数   → 1セルの中に行ごとのラベルが並んでいる（「Scope 1／2／3」、
    ...
    for (ti, ri) in sorted(buckets):
        # 同じ列の中は上から。上付き文字（「m3」の 3）は y0 が少し上にあるので、
...
def apply_table_splits(table_rows: list[dict], st: Settings,
    ...
    """表の行ブロックを、行（セル断片）の境目で2つに分ける（`apply_splits` の表版）。

    背景：KPI表では、1つの行の中に「ラベル＋箇条書き」のセル群が目標側と実績側の2つ連結され、
    行の鍵になるラベルが行内で再掲されることがある。行が実質、並列したサブ表の連結に
    なっているケースは、セル群の境目で分けて「ヒットを含むセル群＝抽出単位」とする
    （単一セル内の箇条書きは行全体のまま）。

    ルールの形は splits と同じ `{"page", "text", "line", "reason"}`（設定JSONも同じ場所）。
    注意: 照合の `text` は行の raw。通常ブロックと違い、表の raw は列の組み直しで
    作られるので行textの連結とは一致しない。だから分けた後の raw も同じ組み直し
    （_row_raw）で作り直す。`line` は前半に残す行数（行ブロック内の並び＝列→上から
    の1始まりで、line 行目までが前半・その次からが後半）。分けた両方の断片に続けて
    照合するので、2本目のルールで3つ以上に分けることもできる（apply_splits と同じ流儀。
    前半にも照合を続ける＝操作の順序に依存しない）。
...
# PDFの中では完全に別のブロックなので ④ の規則では繋がらず、句点が補われて1つの文が2つの
# 「文」になる。手で `⬆ 上と結合` していては、数十冊の規模では回らない。
#
...
#      注意: 1行しか無いブロック（見出し・ラベル・箇条書き）は右端が分からないので繋がない
...
# 注意: 規則なので外れることはある。外したいときは `auto_join` を切る（文書全体）。
...
def auto_join_groups(groups: list[dict], st: Settings) -> list[dict]:
    ...
    for g in groups:
        ...
        # 全単位が除外されたブロックは透過して遡る（→ apply_joins の説明）
        if g.get("全除外"):
...
def analyze_page(page, st: Settings, body: float, pageno: int,
    ...
    手で除外した単位は消さずに `excluded` 印を付けて返す。画面で戻せるようにするため。
    """
    ...
    for g in line_groups:
        ...
        # 注意: strip しない。段をまたぐ結合で `「Prosperity ` の末尾の空白が要るため
        raw = "".join(ln["text"] for ln in g)
        auto = kind_of(size, body, st)
        # 注意: 種別の指定は結合前のブロックを名指しする（キーは後の `parts[0]`）。
        ...
        groups.append({
            ...
            "forced_kind": forced,
            "size": size,
            # text も持たせる。語ハイライトの「どの行の出現か」の照合に使う。
    ...
    for tr in table_rows:
        ...
        groups.append({
            ...
            "forced_kind": forced,
            "size": size,
            # 注意: 表の raw は列の組み直しで作られるので、行の text の連結とは一致しない
            ...
            # 並べ替えの鍵は表の左端にする。行ごとに最初のセルが空だと左端がばらつき、
    ...
    # 注意: 二重描画の除去 → 自動の並べ替え → 手の並べ替え → 結合、の順。
    ...
    for gid, g in enumerate(groups):
        ...
        # （結合したブロックの除外を戻せなくなる）。
        g["bbox"] = _bbox(g)
        # 注意: pt と位置も渡す。同じページに同じ文言・同じptのブロックが複数あるため
        #       （→「ブロックを1つだけ名指しするための鍵」）
        ...
        g.update({
            ...
            # 注意: 一部だけ除外された場合は、どの行が該当するかまでは特定できない
            ...
            # 注意: 手で `大` にしたものは pt の条件を免除する。ptで判定を外したブロックを
...
def page_label(page, pageno: int) -> str:
    ...
    注意: `<FEFF0043>1` のように生の16進文字列で返るPDFがある（UTF-16BE の PDF 文字列を
...
def extract_doc(doc, st: Settings, body: float | None = None) -> list[dict]:
    ...
    注意: PDFのしおり（get_toc）が入っていれば正確に取れるが、0件の文書が多いのでこの推定に頼る。
          セクション列は「参考値」であって、正確な章構造ではない。
    """
    if body is None:
        body = st.body_size if st.body_size else detect_body_size(doc)
    # ページ除外（skip_pages）は適用しない。分母（総文数・総ページ数）を全ページ・全文で
    # 数え、人の判断を分母から排除するため。表紙・目次などにヒットが出た場合は
    # L2 の unit_excludes（理由コード付き）で単位側から外す
        ...
        for g in groups:
            ...
            for k, u in enumerate(g["units"]):
                ...
                rows.append({
                    ...
                    "ブロック": bid,     # 注意: CSVの列にはしない（fieldnames に無いので落ちる）
                })
                # 持ち越し候補＝このページで最後の、2行以上ある本文ブロック。
                # 注意: 1行の本文（ページ右端のナビ項目・1行の見出し）は候補にしない。
...
def aggregate_pages(rows: list[dict]) -> list[dict]:
    ...
    注意: 文が1つも出なかったページ（除外ページ・図だけのページ）は行にならない。
...
def kh_text(rows: list[dict], title: str) -> str:
    ...
    注意: 見出し（h1/h2）の文字列は KH Coder 3.Alpha.7 以降、語の集計に入らない（旧掲示板 No.3231）。
    注意: `<h2>` はそのページに文があるときだけ出す。`aggregate_pages` の行と同じ順・同じ数になる。
    注意: ページをまたいで繋いだ文は、始まったページの行に入っている（→ extract_doc）。
    """
    out = [f"<h1>{title}</h1>"]
    last = None
    for r in rows:
...
# --- 文脈窓（補助） -------------------------------------------------------
# 「生成AI」を含む文の前後N文を1つの窓にする。文（狭い）とページ（広い）の間を
# 固定幅で見るためのもの。段落のような「意味のまとまり」は作らない。恣意性は N の1つだけ。
#
# 検索語。何を「言及」とみなすかの定義そのものなので、変えたら記録に残すこと。画面からも変えられる
...
def keyword_regex(keywords: list[str] | None = None) -> "re.Pattern":
    ...
    if not kws:
        kws = list(KEYWORDS)
    # 注意: 英字だけの語（LLM・GenAI）は単語として当てる。そうしないと「Fulfillment」の中の
    #       "llm" に当たってしまう。日本語を含む語はそのまま部分一致
    parts = []
    for k in kws:
        e = re.escape(k)
...
# 手作業は2種類だけ：足す（unit_merges）と外す（unit_excludes・理由コード必須級）。
# どちらも全件が設定JSONに残り、再生すれば同一の出力になる。
# 注意: 出現率は L2 で数えない。手作業で件数が動くと率が手続きに依存するため、
...
def extract_units(rows: list[dict], keywords: list[str] | None = None,
    ...
    for i in sorted(hit_set):
        ...
        # 注意: 既に別の単位に属している文は足さない。足した文が検索語の追加で自分もヒットに
        #       なると、「自分の単位」と「足された先の単位」の両方に載って黙って二重カウントされる。
...
# 抽出単位CSVの列（全文書共通。KH Coder 用の xlsx もこの並び。テキストは最後の列）
...
#
# 数十冊を1ページずつ見て回るのは現実的でないので、人の作業を「選ぶ」から「確かめる」に変える。
# 注意: 候補は提案であって決定ではない。どれも設定JSON（skip_pages）に入るのは人が採用したとき。
#       採用したものには `"auto": true` が付く＝「機械の候補をそのまま採った」と後から分かる。
#
# 手がかりは、ページの位置・文字数・大きな文字の文言だけ（意味は読まない）。
#   表紙       … 1ページ目。最終ページは文字が少なければ裏表紙
#   目次       … 文書の頭のほうで、「目次／CONTENTS／Index」がそのページで一番大きな文字か、
#                行の4割以上が数字で終わる（見出し＋ページ番号の羅列）。
#                注意: 文言だけで判定すると、全ページに「Index」ナビがある文書で目次が量産される
...
def suggest_skips(doc, body: float, st: Settings | None = None) -> list[dict]:
    ...
    for i, page in enumerate(doc):
        ...
        if pageno <= max(10, n * 0.15):
            # 注意: 「目次／CONTENTS／Index」の文言は、そのページで一番大きな文字のときだけ目次とみなす。
            #       全ページの右上に小さな「Index」ナビがある文書では、文言だけで判定すると
            #       十数ページが全部「目次」になってしまう
