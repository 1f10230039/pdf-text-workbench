...
$("auditGuide").onclick = () => {
  modal("判断に困ったときは？ — 単位と操作の判断基準", el(`<div class="note" style="max-width:640px">
    <p><b>大原則は4つ。</b></p>
    <ol>
      <li><b>単位の広さは類型規則が決める</b>（本文の文→その1文／表の行→行ブロック全体／
        図解ラベル・見出し→その1行）。その場の感覚で広げたり狭めたりしない</li>
      <li><b>「当てはまる理由コードが無い」は操作の場面ではないサイン。</b>
        理由を無理にこじつける前に、操作そのものを疑う</li>
      <li><b>外す・分ける前に必ず原本を見る</b>（カード右の原本表示で現物を確認）</li>
      <li><b>同じ型には同じ手続き</b>。特に同じ会社の年違いの同じ表は必ず揃える</li>
    </ol>
    <p><b>よくあるケース：</b></p>
    <ul>
      <li><b>表の1行に目標と実績が両方入っている</b> —
        列見出し（KPI／実績など）を異にする箇条書きセル群の連結なら
        <b>分ける（理由：セル群の癒着）</b>。ヒットを含むセル群が単位になる
        （例：D社 2024 p11／2025 p10 のマテリアリティKPI表）。
        ⚠️ <b>単一セルの中の箇条書きは行全体のまま</b>（例：D社 2024 p12。実績セルに3項目、ヒットは1つ→そのまま）</li>
      ...
      <li><b>リンクにヒット</b> — <b>文の形をした記述か、ラベルか</b>で決める。
        事例・ニュース見出し（主語＋述語がある）なら<b>残す</b>（例：D社「130億パラメータで…LLMを開発」）。
        ページ名・ボタン文言なら<b>単位ごと外す（理由：リンク）</b>（例：D社「〈製品名〉: 製品・ソリューション」）</li>
      ...
      <li><b>列が混ざる・文の途中に別の欄の文言が割り込む</b> — 表の誤検出。
        <b>表の検出をやめる（理由：レイアウトの枠）</b>で、ページごと取り直す（例：D社 2024 p76・p115）</li>
      <li><b>表なのに指標名と値がバラバラのラベルで拾われる</b> — 罫線なし表の検出漏れ。
        「開いて直す」の抽出タブで<b>表の範囲を手で指定（理由：罫線なし）</b>すると、
        指標名と値が同じ行の単位で出会う（例：A社 2025 p14 のマテリアリティ×非財務指標。
        値のセルはヒットしないので、指定しないと目標値がL2から消える）。
        同じ表の年違いにも同じ指定を</li>
      <li><b>外すときは必ずこの画面の「単位ごと外す」（L2）</b>。
        L1の「除外」はデータごと消えて<b>出現率が動く</b>ので、ヒットの除外には使わない</li>
    </ul>
    <p class="hint">迷って決めた判断は記録に残る（設定JSONの理由コード）。
    新しい型に出会って基準が無いときは、決めてからこの一覧に足すこと。</p></div>`),
  ...
  modal("確認モードでできること", el(`<div class="note" style="max-width:560px">
    ...
    <ul>
      ...
      <li><b>分ける</b> — 見出しが本文と癒着して1つの文になったブロックを、行の境目で2つに分ける。
        切り離した見出しは単独の文（前後の文脈）になり、単位には本文だけが残ります。
        表の行も、1行の中に独立したセル群（ラベル＋箇条書き）が複数連結されているときは
        セル群の境目で分けられます（ヒットを含む側だけが単位として取り直されます）</li>
...
const AU_FALLBACK_REASONS = {
  ...
  splits: [
    ...
    // 2026-09-01 表の行の分割に伴い追加（→ core.OP_REASONS）
...
async function auSplitMenu(it, s, anchor) {
  ...
  if (g.table !== undefined && g.table !== null) {
    ...
    bounds = lines.slice(1).map((_, i) => ({ part: 0, line: i + 1, k: i + 1 }));
  } else {
