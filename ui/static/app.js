  ...
  modal("判断に困ったときは？ — 単位と操作の判断基準", el(`<div class="note" style="max-width:640px">
    ...
    <ul>
      ...
      <li><b>段またぎで文が切れて、間にページ番号・柱が挟まる</b> —
        「前と繋ぐ」は<b>1つ前のブロック</b>としか繋がらないので、挟まったページ番号を巻き込んで誤爆する
        （座標カット廃止後はページ番号がデータに残るため）。L1で戦わず、
        <b>断片になった2つの単位を「足す（理由：文の続き）」で束ねる</b>のが早い
        （例：T社 2024 p46「また、生成AIによ。」＋「る回答後も…」。
        途中に機械が補った句点が残るが、集計は単位ごとなので影響しない）</li>
      <li><b>※・＊の注記にヒット</b> — 形式ではなく<b>中身</b>で決める。
        設立年月・出典・案内のような補足（本体の記述は別の単位で拾えている）なら
        <b>外す（理由：出典注記）</b>（例：A社の「＊1 前身の…室を設立」）。
        <b>注記にしか書かれていない実質情報</b>（投資額など）なら<b>残す</b>
        （例：T社 2024 p10「SuperPODなど生成AI関連の投資73億円」＝生成AI投資の唯一の言及）</li>
